...
before(async () => { await ensureDatabaseReady(); await createTestUser({ id: USER_ID }); await createTestUser({ id: OTHER_USER_ID }) })
...
test('DELETE de categoria é físico quando livre e é recusado quando há transações', async () => {
  ...
  const impact = await categoryService.getCategoryDeletionPreview(used.id)
  assert.deepEqual({ transactions: impact.transactions, recurringExpenses: impact.recurringExpenses, canDelete: impact.canDelete }, { transactions: 1, recurringExpenses: 0, canDelete: false })
...
test('DELETE de recorrência é físico e pode preservar ocorrências como histórico independente', async () => {
  ...
  const preview = await recurringService.getRecurringExpenseDeletionPreview(recurring.id)
  assert.equal(preview.transactionCount, 1)
  await assert.rejects(() => require('../src/services/recurringExpense.service').getRecurringExpenseDeletionPreview(OTHER_USER_ID, recurring.id), NotFoundError)
  await recurringService.deleteRecurringExpense(recurring.id, { mode: 'preserve-history' })
  assert.equal(await getDatabase().prepare('SELECT id FROM recurring_expenses WHERE id = ?').get(recurring.id), undefined)
  assert.equal((await getDatabase().prepare('SELECT count(*) AS count FROM transactions WHERE recurring_expense_id IS NULL AND description = ?').get('Serviço')).count, 1)
})

test('DELETE de recorrência com histórico apaga somente as ocorrências daquele usuário', async () => {
  const expenseCategory = await category('Recorrência destrutiva')
  const recurring = await recurringService.createRecurringExpense({ description: 'Apagar tudo', amount: 25, categoryId: expenseCategory.id, dayOfMonth: 5, startDate: '2030-01-01', endDate: null, isActive: true, cardId: null, notes: '' })
  await recurringService.ensureRecurringExpensesGenerated({ asOfDate: '2030-03-20' })
  const result = await recurringService.deleteRecurringExpense(recurring.id, { mode: 'with-history' })
  assert.equal(result.deletedTransactions, 3)
  assert.equal((await getDatabase().prepare('SELECT COUNT(*) AS count FROM transactions WHERE recurring_expense_id = ?').get(recurring.id)).count, 0)
})
...
test('categoria e cartão vinculados a uma regra recorrente não podem ser apagados', async () => {
  ...
  const categoryImpact = await categoryService.getCategoryDeletionPreview(expenseCategory.id)
  const cardImpact = await cardService.getCardDeletionPreview(card.id)
  assert.equal(categoryImpact.recurringExpenses, 1)
  assert.equal(categoryImpact.canDelete, false)
  assert.equal(cardImpact.recurringExpenses, 1)
  assert.equal(cardImpact.canDelete, false)
